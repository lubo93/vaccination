...
IFR2IFR1_arr = np.linspace(0, 1, 30)
...
IFR2IFR1, BETA2BETA1 = np.meshgrid(IFR2IFR1_arr,beta2beta1_arr)

nu_max = 1e-3
beta = 3/14
IFR1 = 0.8*1e-2

for (ifr2ifr1,beta2beta1) in zip(np.ravel(IFR2IFR1),np.ravel(BETA2BETA1)):
    ...
    1e-2, 3e-3, 1/14, 2/14, 4/14, 1/5, 1/5, 1/5, 1e-2, IFR1, IFR1*ifr2ifr1, 21]
...
R_0 = R_0_arr.reshape(IFR2IFR1.shape)   

f = f_arr.reshape(IFR2IFR1.shape)    
...
midnorm = MidpointNormalize(vmin=-0.005, vcenter=0, vmax=0.015)
...
cm1 = ax[0].pcolormesh(IFR2IFR1, BETA2BETA1, f, cmap=cmap, alpha = 1, linewidth=0,  \
antialiased=True, norm=midnorm)

ax[0].set_xlabel(r"$f^{\star \star}/f^{\star}$")
ax[0].set_ylabel(r"$\beta_2/\beta_1$")

ax[1].set_title(r"$\Delta(D_1,D_2)=(D_2-D_1)/\mathrm{max}(D_1,D_2)$")
cm2 = ax[1].pcolormesh(IFR2IFR1, BETA2BETA1, F, cmap=cmap, alpha = 1, linewidth=0,  \
antialiased=True, norm=midnorm)
ax[1].set_xlabel(r"$f^{\star \star}/f^{\star}$")
...
plt.colorbar(cm2, ax=ax[1], shrink=0.9, ticks = [-0.005, 0, 0.005, 0.01, 0.015])

plt.savefig("beta2beta1_IFR2IFR1_1.png", dpi = 300)
