...
beta_arr = np.linspace(1/14, 4/14, 10)
eta2 = 3e-3
eta1eta2_arr = np.linspace(-eta2,1e-1, 10)
...
for (beta,eta_diff) in zip(np.ravel(BETA),np.ravel(ETA1ETA2)):
    ...
    eta_diff+eta2, eta2, 1/14, 2/14, 4/14, 1/5, 1/5, 1/5, 1e-2, 5e-3, 1e-3, 21]
    ...
    if model1.reproduction_number >= 2 and eta_diff >= 0.02:
...
cm1 = ax[0].pcolormesh(R_0, ETA1ETA2, f, cmap=cmap, alpha = 0.8, linewidth=0, antialiased=True)

ax[0].set_xlabel(r"$R_0$")
ax[0].set_ylabel(r"$\eta_1-\eta_2$")

ax[1].set_title(r"$\Delta(D_1,D_2)=(D_2-D_1)/\mathrm{max}(D_1,D_2)$")
cm2 = ax[1].pcolormesh(R_0, ETA1ETA2, F, cmap=cmap, alpha = 0.8, linewidth=0, antialiased=True)
